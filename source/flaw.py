...
def create_reconstructed(file :Teams_and_Meta ,number_of_flawed : int,chance_of_missing_points : int = None,max_vary_amount=None,chance_to_vary : int = None, confidence_multiplier=1.0, bayesian = False) -> Teams_and_Meta:
    ...
    for i in range(len(flawed_files)-1):
        flawed_files[0].combine(flawed_files[i+1], fill=True, bayesian=bayesian)
